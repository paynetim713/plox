...
(() => {
  ...
  function reset(){
    ...
    freezeT=0; junkWarn.length=0; clearTimer=0; flashPulse=0; rotT=0;   // 清掉上一局残留的瞬时特效/计时,避免开局卡顿+幻影预警
  ...
  function gameOver(){
    ...
    state="gameover"; stopMusic(); softDrop=false; CG.gameplayStop(); beep(140,.25,"sawtooth",.15);
    if(score>high){ high=score; setBest(diffKey,score); elHigh.textContent=high; CG.happytime(); }   // 个人最高本地更新(不强制上传)
  ...
  function doRevive(){
    ...
    spawn();
    if(state==="playing"){ last=performance.now(); startMusic(); CG.gameplayStart(); }
  }
  ...
  function lockPiece(){
    ...
    spawnParticles(current.col, lowR, current.colors[current.colors.length-1], 5);   // 落地小尘
    shakeT=Math.max(shakeT, 38);
    beep(150,.05,"square",.07); beep(95,.08,"sine",.05);   // 两段"咚",比乱入落地轻
  ...
  function hardDrop(){ if(!playing())return; let n=0; while(valid(current.row+1,current.col,current.colors)){current.row++;n++;} visRow=current.row; score+=n*2; syncHUD();
    shakeT=Math.max(shakeT, Math.min(64, 40+n*4)); freezeT=Math.max(freezeT, 18);   // 砸得越远越重(仍轻于乱入落地)
    lockPiece(); }
  ...
  function updateFx(dt){
    ...
    for(let i=junkWarn.length-1;i>=0;i--){ junkWarn[i].t+=dt;   // 预警保留到「计时到 + 该列已无下落乱入」,整个下落过程都有落点锚
  ...
  function drawBlock(g, colF, rowF, idx, opt, cell){
    ...
    if(opt.active){   // 正在操作的方块:加一圈本色亮边,和已落定的堆叠区分开
  ...
  function render(){
    ...
    // 移动端「下一个」预览:先画(放在乱入预警/落块之下,避免盖住第 6 列的预警与下落块)
  ...
  document.addEventListener("visibilitychange",()=>{
    ...
    else if(state==="playing"){ ensureAudio(); startMusic(); CG.gameplayStart(); }
  });
  ...
  // ---------- 触摸操作:轻点=旋转,左右拖=逐格移动(对起点累计),向下滑=直接落地。已去掉「慢拖加速」。----------
  // 一次手势只走一个方向(横/竖二选一):竖直手势绝不横移,所以「向下滑落地」一定落在落点预览处,不会偏。
  ...
  cv.addEventListener("touchmove",e=>{ e.preventDefault();
    if(!tS||!playing()||e.touches.length>1){ return; }
    const t=e.touches[0], dx=t.clientX-tS.x, dy=t.clientY-tS.y, adx=Math.abs(dx), ady=Math.abs(dy);
    if(!tS.axis && (adx>CELL*0.4 || ady>CELL*0.4))
      tS.axis = (ady > adx*1.5) ? "v" : "h";   // 需更明确的竖直分量才算下滑;否则一律横移 → 横移更跟手
    if(tS.axis==="h"){                          // 仅横向手势才移动;竖向手势绝不横移
      const want=Math.round(dx/CELL);           // 对起点累计:约半格即移一格,贴手不漂移
  ...
  cv.addEventListener("touchend",e=>{ e.preventDefault(); if(e.touches.length>0){ tS=null; return; } if(!tS)return;
    const t=e.changedTouches[0], dx=t.clientX-tS.x, dy=t.clientY-tS.y, now=performance.now();
    if(tS.axis==="v" || (tS.axis!=="h" && dy>CELL*0.5 && dy>Math.abs(dx)*1.2)){   // 任何明确的向下手势 = 落到预览处
      if(now-lastHardT>220){ lastHardT=now; hardDrop(); }   // 去抖:快速下甩的回弹/二次触点不再误触发第二次落地
    } else if(!tS.moved) rotate();   // 没发生横移的点/微动 = 旋转(消除抖动死区)
