...
(() => {
  ...
  let tS=null;
  cv.addEventListener("touchstart",e=>{ e.preventDefault(); if(e.touches.length>1){ return; } ensureAudio();
  ...
  cv.addEventListener("touchmove",e=>{ e.preventDefault();
    ...
    if(!tS.axis && (adx>CELL*0.4 || ady>CELL*0.4))
      tS.axis = (ady > adx*0.8) ? "v" : "h";   // 偏向竖直:下滑不会被误判成横移(避免落点偏移)
    if(tS.axis==="h"){                          // 仅横向手势才移动;竖向手势完全不横移
      const want=Math.round(dx/CELL);           // 对起点累计:约半格即移一格,贴手不漂移
      while(tS.steps<want){ move(1); tS.steps++; tS.moved=true; }
      while(tS.steps>want){ move(-1); tS.steps--; tS.moved=true; }
    } },{passive:false});
  cv.addEventListener("touchend",e=>{ e.preventDefault(); if(!tS)return;
    const t=e.changedTouches[0], dx=t.clientX-tS.x, dy=t.clientY-tS.y;
    if(tS.axis!=="h" && dy>CELL*1.2 && dy>Math.abs(dx)*1.4) hardDrop();   // 向下滑(非横移手势)= 落到预览处,快慢都行
    else if(!tS.axis && Math.abs(dx)<CELL*0.4 && Math.abs(dy)<CELL*0.4) rotate();   // 轻点 = 旋转
